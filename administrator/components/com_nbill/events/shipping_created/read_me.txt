To execute a PHP script when this event is fired, just save a PHP file in this folder and it will be included when the event occurs. Files are executed in alphabetical order.

EVENT:
shipping_created

PARAMS:
["id"] => int (ID number of the newly created shipping record. You can look up the rest of the details on the #__nbill_shipping and #__nbill_shipping_price tables)
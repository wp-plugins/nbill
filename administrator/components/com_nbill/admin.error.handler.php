<?php
/**
* Main administrator error handler
* @version 2
* @package nBill
...
(defined('_VALID_MOS') || defined('_JEXEC') || defined('ABSPATH') || defined('NBILL_VALID_NBF')) or die('Access Denied.');
	...
	set_error_handler("nbill_admin_error_handler");
	nbf_globals::$fatal_error_handled = false;

	function fatal_error_handler_admin($buffer)
	{
		if(error_reporting() != 0)
		{
			restore_error_handler();
			$error_pos = nbf_common::nb_strpos($buffer, "<b>Fatal error</b>:");
			if ($error_pos !== false && !nbf_globals::$fatal_error_handled)
			{
				//Parse output for error details ... eg.
				//<b>Fatal error</b>:  Call to undefined function:  madeupfunction() in <b>W:\www\username\administrator\components\com_nbill\admin.nbill.php</b> on line <b>111</b><br />
				...
				nbf_globals::$fatal_error_handled = true; //Don't try to handle it again when the buffer gets cleared
				$result = nbill_admin_error_handler(E_ERROR, $error_str, $error_loc, $error_line);
                return $result;
			}
			else
			{
	...
	function nbill_admin_error_handler($errno, $errstr, $errfile="", $errline=-1, $errcontext = array())
	{
		...
		if(error_reporting() != 0)
		{
			...
			if ((nbf_common::nb_strlen($errfile) > 0 && (nbf_common::nb_strpos($errfile, "com_nbill") !== false || nbf_common::nb_strpos($errfile, NBILL_BRANDING_COMPONENT_NAME) !== false)) &&
                    $errfile != nbf_cms::$interop->nbill_admin_base_path . "/admin.error.handler.php" &&
					$errfile != nbf_cms::$interop->nbill_admin_base_path . "\\admin.error.handler.php")
			{
				...
				switch($errno)
                {
                    ...
                    case E_ERROR:
                        ...
                        if (method_exists($database, "setQuery")) //Reference to $database is deliberate (NOT $nb_database, which will always exist!)
                        {
	                    ...
	                    nbf_globals::$message = @NBILL_ERR_REPORT_INTRO_ADMIN;
	                    if (nbf_common::nb_strlen(nbf_globals::$message) == 0 || nbf_globals::$message == "NBILL_ERR_REPORT_INTRO_ADMIN")
	                    {
		                    nbf_globals::$message = "An error has occurred in " . NBILL_BRANDING_NAME . " (administrator)! Details of the error are given below:\n\n";
	                    }
                        ...
                        if (file_exists(realpath(dirname(__FILE__)) . '/framework/classes/nbill.version.class.php')) {
                            include_once(realpath(dirname(__FILE__)) . '/framework/classes/nbill.version.class.php');
	                    ...
	                    nbf_globals::$message .= "Page: " . @nbf_common::get_requested_page() . "\n";
                        ...
                        if (!function_exists("obsafe_print_r"))
                        {
                            ...
                            * added by Fredrik Wollsen (fredrik dot motin at gmail), to make it work as a drop-in
                ...
                switch ($errno)
                {
                    ...
                    case E_ERROR:
                        if ($mailsent)
                        {
	                        $errmsg = @NBILL_ERR_SERIOUS_ERROR_ADMIN;
	                        if (nbf_common::nb_strlen($errmsg) == 0 || $errmsg == "NBILL_ERR_SERIOUS_ERROR_ADMIN")
	                        {
		                        $errmsg = "Sorry, an error has occurred. An e-mail containing details of the error has been sent to the appropriate person (as defined in the Global Configuration page of " . NBILL_BRANDING_NAME . "). Apologies for the inconvenience.";
	                        }
                        }
                        else
                        {
                            $errmsg = @sprintf(NBILL_ERR_SERIOUS_ERROR_NOMAIL_ADMIN, nbf_cms::$interop->live_site);
	                        if (nbf_common::nb_strlen($errmsg) == 0 || $errmsg == "NBILL_ERR_SERIOUS_ERROR_NOMAIL_ADMIN")
	                        {
		                        $errmsg = "Sorry, an error has occurred. " . NBILL_BRANDING_NAME . " was unable to send an e-mail containing details of the error to the appropriate person (as defined in the Global Configuration page of " . NBILL_BRANDING_NAME . "). As this error occurred in the administrator, details of the error are provided below. Apologies for the inconvenience.";
	                        }
	                        $errmsg .= "<br /><br />" . str_replace("\n", "<br />", nbf_globals::$message);
                        }

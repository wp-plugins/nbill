<?php
/*
You can create your own quote template based on this one, but if you want to
...
(bear in mind that this file is included once for each quote).

CSS class definitions must be stored in a separate file called 'template.css'
...
However, you can add styling in here if it is only going to be used for HTML
previews and you don't mind invalid HTML - just wrap it in a php condition
(checking the $pdf flag). Inline style attributes are ok for both HTML and PDF.
...
$document_items (array) Array of objects containing information about the individual items that make up the quote
        ...
        ->quote_pay_freq (string) Payment frequency code for the item
        ->quote_relating_to (string) Relating to value that will appear on order/invoice
        ->quote_item_accepted (int) Boolean int (1 for yes, 0 for no) to indicate whether client has accepted this part of the quote
        ...
        ->section_quote_atomic (int) Boolean int (1 for yes, 0 for no) to indicate whether all items in the section have to have the same status (accepted or rejected)
...
of that, you can simply override the nBillQuoteLineItemHtml class (an example of this is provided with the default template in
the file default_template_quote_line_item.php).
*/
...
//We have to use inline styling for the sake of legacy PDF generation (also helps with some webmail clients)
if ($pdf) {
    ...
    $style_container = "font-family: firefly, DejaVu Sans, Tahoma, Verdana, Arial, Helvetica, sans-serif;font-size: 10pt;padding-left:4%;padding-right:4%;";
} else {
    $style_container = "font-family: Tahoma, Verdana, Arial, Helvetica, sans-serif;font-size: 10pt;padding-left:4%;padding-right:4%;";
}
...
$style_td = "border:solid 1px #aaaaaa;border-top:0px;text-align:left;";
...
$style_accepted_total_td = "border:solid 1px #aaaaaa;border-top:0px;text-align:left;font-weight: bold;color: #009900;";
$style_accepted_total_td_top = "border:solid 1px #aaaaaa;text-align:left;font-weight: bold;color: #009900;";
$style_accepted_total_td_numeric = "border:solid 1px #aaaaaa;border-left:0px;border-top:0px;text-align:right;white-space:nowrap;font-weight: bold;color: #009900;";
$style_accepted_total_td_numeric_top = "border:solid 1px #aaaaaa;border-left:0px;text-align:right;white-space:nowrap;font-weight: bold;color: #009900;";
...
$line_item_style_array = array($style_th, $style_th_numeric, $style_th_left, $style_th_section_header,
        ...
        $style_accepted_total_td_numeric, $style_accepted_total_td_numeric_top);
?>
...
<div class="document-container" style="<?php echo $style_container; ?>">
    ...
    <table class="billing-details" style="<?php echo $style_billing_details; ?>">
        <tr>
            ...
            <td class="vendor" style="<?php echo $style_vendor; ?>">
                <h1 style="<?php echo $style_title; ?>"><?php echo nbf_common::parse_translation($document->default_language, "template.qu", 'NBILL_PRT_QUOTE_TITLE'); ?></h1>
        ...
        <tr>
            ...
            <td>
                <table class="document-details-table">
                    <tr>
                        <td style="<?php echo $style_document_details_td; ?>"><?php echo nbf_common::parse_translation($document->default_language, "template.qu", 'NBILL_PRT_QUOTE_NO'); ?>&nbsp;</td>
                        <td style="<?php echo $style_document_details_td; ?>"><?php echo $document->document_no; ?></td>
                    </tr>
                    <?php if (strlen($document->reference) > 0) { ?>
                    <tr>
                    ...
                    <?php if (strlen($document->tax_no) > 0) { ?>
                    <tr>
                    ...
                    <?php } ?>
                    <?php if (strlen($document->tax_exemption_code) > 0) { ?>
                    <tr>
                    ...
                    <?php } ?>
                    <tr>
                        <td style="<?php echo $style_document_details_td; ?>"><?php echo nbf_common::parse_translation($document->default_language, "template.qu", 'NBILL_PRT_QUOTE_STATUS'); ?>:&nbsp;</td>
                        <td style="<?php echo $style_document_details_td; ?>"><?php echo @constant($document->quote_status_desc); ?></td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    echo $doc_head;
    if (strlen(trim(strip_tags($document->quote_intro))) > 0)
    {
        ?>
        <p class="quote-intro"><?php echo $document->quote_intro; ?></p>
        <?php
    }
    $document->language = $document->default_language; //For forward compatability
    $line_item_view = new nBillDefaultTemplateQuoteLineItem($line_items[$document->id], $document);
    ...
    if (strlen(trim($document->correspondence)) > 0)
    { ?>
        <br /><br /><hr />
        <h3><?php echo nbf_common::parse_translation($document->default_language, "template.qu", 'NBILL_QUOTE_CORRESPONDENCE_INTRO'); ?></h3>
